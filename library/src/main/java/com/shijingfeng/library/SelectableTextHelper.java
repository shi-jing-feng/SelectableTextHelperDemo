...
import com.shijingfeng.library.entity.Cursor;
import com.shijingfeng.library.entity.SelectionInfo;
import com.shijingfeng.library.util.ScreenUtil;
...
public class SelectableTextHelper {
    ...
    /** 开始和结束游标信息 和 选择文本信息 清除后的缓存 (用于点击操作框关闭后，保存数据信息) */
    ...
    private SelectableTextHelper(@NonNull Builder builder) {
        ...
        this.mCursorHandleSize = ScreenUtil.dp2px(mContext, builder.mCursorHandleSizeInDp);
...
//        });
    ...
    private void showSelectView() {
        ...
        //此处可能会有问题（如果父布局不是滑动布局，父布局的父布局是滑动布局）
    ...
    public CharSequence getSelectedCharSequence() {
        final SelectionInfo currentSelectionInfo;

        if (mSelectionInfo.isClear()) {
            currentSelectionInfo = mSelectionInfoCache;
            mSelectionInfoCache = null;
        } else {
            currentSelectionInfo = mSelectionInfo;
        }

        int startIndex = currentSelectionInfo.startCursor.offset - 1;
        int endIndex = currentSelectionInfo.endCursor.offset - 1;
...
//        Runtime.getRuntime().gc();
    ...
    private class CursorHandle extends View {
        ...
        private void update(final float dx, final float dy) {
            ...
            if (mTextView.getParent() != null) {
                textViewParent = ((ViewGroup) mTextView.getParent());
                parentWidth = textViewParent.getWidth() - mTextView.getPaddingLeft() - mTextView.getPaddingRight();
                parentHeight = textViewParent.getHeight() - mTextView.getPaddingTop() - mTextView.getPaddingBottom();
            }
            ...
            if (newX - mScrollX < 0) {
                updateX = false;
            } else if (newX - mScrollX > parentWidth) {
                updateX = false;
            }
    ...
    private class OperatePopupWindow {
        ...
        private final float ELEVATION_WIDTH = ScreenUtil.dp2px(mContext, 2);
        ...
        public void show() {
            ...
            if (x < 0) {
                x = 0;
            } else if (x > ScreenUtil.getScreenWidth(mContext) - mWindowViewWidth - 2 * ELEVATION_WIDTH) {
                x = (int) (ScreenUtil.getScreenWidth(mContext) - mWindowViewWidth - 2 * ELEVATION_WIDTH);
            }
            if (y < 0) {
                y = 0;
            } else if (y > ScreenUtil.getScreenHeight(mContext) - mWindowViewHeight - 2 * ELEVATION_WIDTH) {
                y = (int) (ScreenUtil.getScreenHeight(mContext) - mWindowViewHeight - 2 * ELEVATION_WIDTH);
            }
